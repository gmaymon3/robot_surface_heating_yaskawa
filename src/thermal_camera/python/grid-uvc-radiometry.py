...
def display_temperature(img, val_k, loc, color):
  val = ktoc(val_k)
  cv2.putText(img,"{0:.1f} degC".format(val), loc, cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
  x, y = loc
...
def main():
  ...
  try:
    ...
    try:
      ...
      y_start = 200
      y_end = 400
      x_spacing = 200  # Adjust this to change the spacing between points
      y_spacing = 100  # Adjust this to change the spacing between points
      ...
      # Bottom Left Corner 
      # (66, 290)
      orig_pts = np.float32([[78, 354], [502, 378], [185, 254],
                       [450, 265]])
      ...
      try:
        while True:
          ...
          for i in range(len(X_flat)):
            flat_values.append(data[X_flat[i],Y_flat[i]])

          img = raw_to_8bit(data)
          for j in range(len(X_flat)):
           color = generate_color(flat_values[j])
           #print(color)
           display_temperature(img, flat_values[j], (X_flat[j],Y_flat[j]), color)
